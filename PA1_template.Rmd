...
####################################################################################
 date <- factor(c("10/1/2002","10/10/2002","10/11/2002","10/12/2002","10/13/2002"))
 stepSumNew <- c(10766.19, 9900.00,10304.00, 17382.00, 12426.00)
 actSummNew <- data.frame(date,stepSum) 
...
str(actSummNew)
actSummNew
actSummNew$date <-as.Date(actSummNew$date, format="%m,/%d,/%Y")
head(actSummNew)


actSummNew$wday <- weekdays(as.Date(actSummNew$date))
head(actSummNew)
head(actSumm)
...
actSummNew$wdayF <- as.factor(c("weekend", "weekday"))
act_new[actSummNew$wday == "Sunday" | actSummNew$wday == "Saturday" ,5]<- factor("weekend")
act_new[!(actSummNew$wday == "Sunday" | actSummNew$wday == "Saturday"),5 ]<- factor("weekday")
